...
public class BlackJack extends Application {
    ...
    // Referenz auf die Stage, um sie später schließen zu können
    ...
    private void updateCardDisplay() {
        ...
        if (!spielerTurn) {
            if (!computerHand.isEmpty()) {
                for (int i = 0; i < computerHand.size(); i++) {
                    ...
                    cardView.setY(215);
                    Text computerText = new Text(291 + hintegrundbildStartWert + (i * 105), 215, computerHand.get(i).wert + " " + computerHand.get(i).typ);
                    root.getChildren().addAll(cardView, computerText);
                }
            }
        } else {
        ...
        if (!spielerHand.isEmpty()) {
            for (int i = 0; i < spielerHand.size(); i++) {
                ...
                cardView.setY(730);
                Text spielerText = new Text(291 + (i * 105), 730, spielerHand.get(i).wert + " " + spielerHand.get(i).typ);
                root.getChildren().addAll(cardView, spielerText);
            }
        }
    }
    ...
    private int countHand(List<Karte> kartenHand) {
        ...
        if (kartenHand.get(0).name.substring(kartenHand.get(0).name.indexOf(" ") + 1).equals("Ass") ||
                kartenHand.get(1).name.substring(kartenHand.get(1).name.indexOf(" ") + 1).equals("Ass")) {
            sonderRegel = true;
        }
    ...
    // --- Neue Methoden gemäß den Anforderungen ---

    /**
     * Zeigt den Hintergrund sowie eine offene Dealer-Karte (erste Karte) und eine verdeckte Karte an.
     */
    public void show() {
        // Alle Nodes entfernen, dann den Hintergrund und die Dealer-Karten hinzufügen.
        ...
        // Offene Karte (erste Karte, sofern vorhanden)
        if (!computerHand.isEmpty()) {
            ImageView openCard = new ImageView(computerHand.get(0).bild);
            openCard.setX(291 + hintegrundbildStartWert - 25);
            openCard.setY(215);
            openCard.setFitHeight(150);
            openCard.setFitWidth(100);
            root.getChildren().add(openCard);
        }
        // Verdeckt angezeigte Karte
        ImageView backCard = new ImageView(karteUmgedrehtHand);
        backCard.setX(291 + hintegrundbildStartWert + 83);
        backCard.setY(215);
        backCard.setFitHeight(150);
        backCard.setFitWidth(100);
        root.getChildren().add(backCard);
    }
    ...
    /**
     * Fügt eine gegebene Dealer-Karte (übergeben als Kartenobjekt) an einer sinnvollen Position hinzu.
    ...
    /**
     * Schließt nur dieses Fenster und beendet damit ausschließlich diese Anwendung.
     */
    public void close() {
